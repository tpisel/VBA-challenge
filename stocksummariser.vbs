Sub stocksummariser_onesheet()
    stocksummariser
End Sub

Sub stocksummariser_allsheets()

    ' This version will loop through all sheets in the work book and apply the changes to each
    Dim ws as Worksheet
...
' the same stocksummariser script
Public Function stocksummariser()
    ...
    dim green_cond as FormatCondition
    dim red_cond as FormatCondition
    
    set green_cond = Range("J:K").FormatConditions.Add(Type:=xlCellValue, Operator:=xlGreater, Formula1:="0")
    set red_cond = Range("J:K").FormatConditions.Add(Type:=xlCellValue, Operator:=xlLess, Formula1:="0")
    Range("J1:K1").FormatConditions.Delete
    
    green_cond.Interior.ColorIndex = 4
    red_cond.Interior.ColorIndex = 3
